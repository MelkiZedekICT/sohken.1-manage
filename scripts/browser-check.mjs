...
await page.locator('#connection').filter({hasText:'Engine connected'}).waitFor();
...
await page.getByRole('button',{name:'Approve exact action'}).last().click();
await page.getByRole('button',{name:'Execute action',exact:true}).last().click();
await page.locator('#notice').filter({hasText:'Execute action completed'}).waitFor();
...
await page.locator('#audit-result').filter({hasText:'Integrity check passed'}).waitFor();
